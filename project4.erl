...
sendHM(AccName, PID) ->
    ...
    .
actors(_PID, AccName, _NoActors, _NoRequests, 0, _Option) -> done;
...
actors(PID, AccName, NoActors, NoRequests, NR, Option) ->
    receive 
        {supSendTweet} -> 
            ...
            receive {sendTwtDone, _Tweet, AccName} -> done
...
startActors(PID, NoActors, NoRequests) ->
    ...
    [spawn(project4, createActors, [PID, AccName, NoActors, NoRequests, NoRequests, 1]) || AccName <- lists:seq(1, NoActors)],
    done.
...
startServer() ->
    ...
    receive {startser, NoActors, NoRequests} ->
        ...
        spawn(project4, supervisor, [ActorsMap1, NoActors, NoActors, (NoActors - 1) * NoRequests, TermMap]),
...
terminateActors(ActorsMap, Len) ->
    ...
    .
server(ActorsMap, _TweetDB, _SubscribedToMap, _NoActors, 0, TL, TL2, TL3, TL4) ->
    io:fwrite("Time taken For Tweeting Tweet: ~p~n", [lists:sum(TL)/ length(TL) * 1000]),
    io:fwrite("Time taken to Subscribe to user:  ~p~n", [lists:sum(TL2)/ length(TL2) * 1000]),
    io:fwrite("Time taken to ReTweet: ~p~n", [lists:sum(TL3)/ length(TL3) * 1000]),
    io:fwrite("Time taken to Query Hashtag/Mentions: ~p~n", [lists:sum(TL4)/ length(TL4) * 1000]),
    terminateActors(ActorsMap, length(maps:keys(ActorsMap))),
    io:fwrite("~nServer Terminating!~n");
server(ActorsMap, TweetDB, SubscribedToMap, NoActors, NoRequests, TL, TL2, TL3, TL4) ->
    receive 
        {sendTwt, Tweet, AccName, CID} ->
            ...
            {_, CPU_Time1} = erlang:statistics(runtime),
            server(ActorsMap, TweetDB1, SubscribedToMap, NoActors, NoRequests - 1, [CPU_Time1] ++ TL, TL2, TL3, TL4);
        {subUser, AccName, CID} -> 
            erlang:system_flag(scheduler_wall_time, true),
            {OtherAccName, SubscribedToMap1} = subscribe(SubscribedToMap, AccName, NoActors),
            CID ! {subUserDone, AccName, OtherAccName}, 
            {_, CPU_Time2} = erlang:statistics(runtime),
            server(ActorsMap, TweetDB, SubscribedToMap1, NoActors, NoRequests - 1, TL, [CPU_Time2] ++ TL2, TL3, TL4);
        {reTwt, AccName, CID} ->
            erlang:system_flag(scheduler_wall_time, true),
            {Tweet, OtherAccName, TweetDB1} = retweet(TweetDB, SubscribedToMap, NoActors, AccName),
            CID ! {reTwtDone, Tweet, AccName, OtherAccName},
            {_, CPU_Time3} = erlang:statistics(runtime),
            server(ActorsMap, TweetDB1, SubscribedToMap, NoActors, NoRequests - 1, TL, TL2, [CPU_Time3] ++ TL3, TL4);
        {queryHM, WhatToSearch, AccName, CID} ->
            erlang:system_flag(scheduler_wall_time, true),
            queryHashtagsMentions(TweetDB, WhatToSearch),
            CID ! {queryHMDone, WhatToSearch, AccName},
            {_, CPU_Time4} = erlang:statistics(runtime),
            server(ActorsMap, TweetDB, SubscribedToMap, NoActors, NoRequests - 1, TL, TL2, TL3, [CPU_Time4] ++ TL4)
    end
